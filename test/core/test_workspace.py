...
class WorkspaceTest(unittest.TestCase):
    def test_utilities(self):
        self.assertEquals(mk_op_arg(1), {'value': 1})
        self.assertEquals(mk_op_arg('2'), {'value': '2'})
        self.assertEquals(mk_op_arg('a'), {'value': 'a'})
    ...
    def test_workspace_can_create_new_res_names(self):
        ...
        self.assertEquals(res_name_1, 'res_1')
        self.assertEquals(res_name_2, 'res_2')
        self.assertEquals(res_name_3, 'res_3')
    ...
    def test_to_json_dict(self):
        ...
        try:
            ...
            self.assertEquals(len(l_res), 10)

            res_ds = l_res[0]
            self.assertEquals(res_ds.get('name'), 'ds')
            self.assertEquals(res_ds.get('dataType'), 'xarray.core.dataset.Dataset')
            self.assertEquals(res_ds.get('dimSizes'), dict(lat=2, lon=2, time=5))
            self.assertEquals(res_ds.get('attributes'), {'history': 'a b c'})
            res_ds_vars = res_ds.get('variables')
            self.assertIsNotNone(res_ds_vars)
            self.assertEquals(len(res_ds_vars), 2)
            res_ds_var_1 = res_ds_vars[0]
            self.assertEquals(res_ds_var_1.get('name'), 'precipitation')
            self.assertEquals(res_ds_var_1.get('dataType'), 'float64')
            self.assertEquals(res_ds_var_1.get('numDims'), 3)
            self.assertEquals(res_ds_var_1.get('shape'), (5, 2, 2))
            self.assertEquals(res_ds_var_1.get('chunkSizes'), None)
            self.assertEquals(res_ds_var_1.get('isYFlipped'), True)
            self.assertEquals(res_ds_var_1.get('isFeatureAttribute'), None)
            self.assertEquals(res_ds_var_1.get('attributes'), dict(x=True, comment='wet', _FillValue=-1.))
            res_ds_var_2 = res_ds_vars[1]
            self.assertEquals(res_ds_var_2.get('name'), 'temperature')
            self.assertEquals(res_ds_var_2.get('dataType'), 'float64')
            self.assertEquals(res_ds_var_2.get('numDims'), 3)
            self.assertEquals(res_ds_var_2.get('shape'), (5, 2, 2))
            self.assertEquals(res_ds_var_2.get('chunkSizes'), None)
            self.assertEquals(res_ds_var_2.get('isYFlipped'), True)
            self.assertEquals(res_ds_var_2.get('isFeatureAttribute'), None)
            self.assertEquals(res_ds_var_2.get('attributes'), dict(a=[1, 2, 3], comment='hot', _FillValue=np.nan))

            res_df = l_res[1]
            self.assertEquals(res_df.get('name'), 'df')
            self.assertEquals(res_df.get('dataType'), 'pandas.core.frame.DataFrame')
            self.assertEquals(res_df.get('attributes'), {'num_rows': 12, 'num_columns': 2})
            res_df_vars = res_df.get('variables')
            self.assertIsNotNone(res_df_vars)
            self.assertEquals(len(res_df_vars), 2)
            res_df_var_1 = res_df_vars[0]
            self.assertEquals(res_df_var_1.get('name'), 'A')
            self.assertEquals(res_df_var_1.get('dataType'), 'float64')
            self.assertEquals(res_df_var_1.get('numDims'), 1)
            self.assertEquals(res_df_var_1.get('shape'), (12,))
            self.assertEquals(res_df_var_1.get('isYFlipped'), None)
            self.assertEquals(res_df_var_1.get('isFeatureAttribute'), True)
            self.assertIsNone(res_df_var_1.get('attributes'))
            res_df_var_2 = res_df_vars[1]
            self.assertEquals(res_df_var_2.get('name'), 'B')
            self.assertEquals(res_df_var_2.get('dataType'), 'float64')
            self.assertEquals(res_df_var_2.get('numDims'), 1)
            self.assertEquals(res_df_var_2.get('shape'), (12,))
            self.assertEquals(res_df_var_2.get('isYFlipped'), None)
            self.assertEquals(res_df_var_2.get('isFeatureAttribute'), True)
            self.assertIsNone(res_df_var_2.get('attributes'))

            res_gdf = l_res[2]
            self.assertEquals(res_gdf.get('name'), 'gdf')
            self.assertEquals(res_gdf.get('dataType'), 'geopandas.geodataframe.GeoDataFrame')
            self.assertEquals(res_gdf.get('attributes'), {'num_rows': 3, 'num_columns': 4, 'geom_type': 'Point'})
            res_gdf_vars = res_gdf.get('variables')
            self.assertIsNotNone(res_gdf_vars)
            self.assertEquals(len(res_gdf_vars), 4)
            res_gdf_var_1 = res_gdf_vars[0]
            self.assertEquals(res_gdf_var_1.get('name'), 'name')
            self.assertEquals(res_gdf_var_1.get('dataType'), 'object')
            self.assertEquals(res_gdf_var_1.get('numDims'), 1)
            self.assertEquals(res_gdf_var_1.get('shape'), (3,))
            self.assertEquals(res_gdf_var_1.get('isYFlipped'), None)
            self.assertEquals(res_gdf_var_1.get('isFeatureAttribute'), True)
            self.assertIsNone(res_gdf_var_1.get('attributes'))
            res_gdf_var_2 = res_gdf_vars[1]
            self.assertEquals(res_gdf_var_2.get('name'), 'lat')
            self.assertEquals(res_gdf_var_2.get('dataType'), 'float64')
            self.assertEquals(res_gdf_var_2.get('numDims'), 1)
            self.assertEquals(res_gdf_var_2.get('shape'), (3,))
            self.assertEquals(res_gdf_var_2.get('isYFlipped'), None)
            self.assertEquals(res_gdf_var_2.get('isFeatureAttribute'), True)
            self.assertIsNone(res_gdf_var_2.get('attributes'))
            res_gdf_var_3 = res_gdf_vars[2]
            self.assertEquals(res_gdf_var_3.get('name'), 'lon')
            self.assertEquals(res_gdf_var_3.get('dataType'), 'float64')
            self.assertEquals(res_gdf_var_3.get('numDims'), 1)
            self.assertEquals(res_gdf_var_3.get('shape'), (3,))
            self.assertEquals(res_gdf_var_3.get('isYFlipped'), None)
            self.assertEquals(res_gdf_var_3.get('isFeatureAttribute'), True)
            self.assertIsNone(res_gdf_var_3.get('attributes'))
            res_gdf_var_4 = res_gdf_vars[3]
            self.assertEquals(res_gdf_var_4.get('name'), 'geometry')
            self.assertEquals(res_gdf_var_4.get('dataType'), 'object')
            self.assertEquals(res_gdf_var_4.get('numDims'), 1)
            self.assertEquals(res_gdf_var_4.get('shape'), (3,))
            self.assertEquals(res_gdf_var_4.get('isYFlipped'), None)
            self.assertEquals(res_gdf_var_4.get('isFeatureAttribute'), True)
            ...
            self.assertEquals(len(res_scalar_ds_vars), 2)
            ...
            self.assertEquals(scalar_values, {'temperature': 15.2, 'precipitation': 10.1})

            res_scalar_df = l_res[4]
            res_scalar_df_vars = res_scalar_df.get('variables')
            self.assertIsNotNone(res_scalar_df_vars)
            self.assertEquals(len(res_scalar_df_vars), 2)
            ...
            self.assertEquals(len(res_empty_ds_vars), 2)
            ...
            self.assertEquals(scalar_values, {'temperature': None, 'precipitation': None})

            res_empty_df = l_res[7]
            res_empty_df_vars = res_empty_df.get('variables')
            self.assertIsNotNone(res_empty_df_vars)
            self.assertEquals(len(res_empty_df_vars), 2)
            ...
            self.assertEquals(scalar_values, {'A': None, 'B': None})

            res_int = l_res[8]
            self.assertEquals(res_int.get('name'), 'i')
            self.assertEquals(res_int.get('dataType'), 'int')
            self.assertIsNone(res_int.get('attributes'))
            self.assertIsNone(res_int.get('variables'))

            res_str = l_res[9]
            self.assertEquals(res_str.get('name'), 's')
            self.assertEquals(res_str.get('dataType'), 'str')
    ...
    def test_set_and_execute_step(self):
        ...
        self.assertEquals('Unknown operation "not_existing_op"', str(we.exception))

        with self.assertRaises(ValidationError) as we:
            ws.set_resource('cate.ops.io.read_netcdf', mk_op_kwargs(location=NETCDF_TEST_FILE_1), res_name='X')
        self.assertEquals('"location" is not an input of operation "cate.ops.io.read_netcdf"', str(we.exception))

        with self.assertRaises(ValidationError) as we:
            ws.set_resource('cate.ops.io.read_netcdf', {'file': {'foo': 'bar'}}, res_name='X')
        self.assertEquals('Illegal argument for input "file" of operation "cate.ops.io.read_netcdf', str(we.exception))
        ...
        self.assertEquals(ws.resource_cache, {})
    ...
    def test_set_and_rename_and_execute_step(self):
        ws = Workspace('/path', Workflow(OpMetaInfo('workspace_workflow', header=dict(description='Test!'))))
        self.assertEquals(ws.user_data, {})
        ...
        self.assertEquals(len(ws.workflow.steps), 3)
        self.assertEquals(ws.resource_cache, {})

        value = ws.execute_workflow('Y')
        self.assertEquals(value, 1)
        self.assertEquals(ws.resource_cache.get('X'), 1)
        self.assertEquals(ws.resource_cache.get('Y'), 1)
        self.assertEquals(ws.resource_cache.get('Z'), None)

        value = ws.execute_workflow('Z')
        self.assertEquals(value, 1)
        self.assertEquals(ws.resource_cache.get('X'), 1)
        self.assertEquals(ws.resource_cache.get('Y'), 1)
        self.assertEquals(ws.resource_cache.get('Z'), 1)

        ws.set_resource('cate.ops.utility.identity', mk_op_kwargs(value=9), res_name='X', overwrite=True)
        self.assertEquals(len(ws.workflow.steps), 3)
        self.assertEquals(ws.resource_cache.get('X'), UNDEFINED)
        self.assertEquals(ws.resource_cache.get('Y'), UNDEFINED)
        self.assertEquals(ws.resource_cache.get('Z'), UNDEFINED)

        ws.execute_workflow()
        self.assertEquals(ws.resource_cache.get('X'), 9)
        self.assertEquals(ws.resource_cache.get('Y'), 9)
        self.assertEquals(ws.resource_cache.get('Z'), 9)

        ws.rename_resource('X', 'A')
        self.assertIsNone(ws.workflow.find_node('X'))
        self.assertIsNotNone(ws.workflow.find_node('A'))
        self.assertEquals(ws.resource_cache.get('X', '--'), '--')
        self.assertEquals(ws.resource_cache.get('A'), 9)
        self.assertEquals(ws.resource_cache.get('Y'), 9)
        self.assertEquals(ws.resource_cache.get('Z'), 9)

        ws.set_resource('cate.ops.utility.identity', mk_op_kwargs(value=5), res_name='A', overwrite=True)
        self.assertEquals(ws.resource_cache.get('X', '--'), '--')
        self.assertEquals(ws.resource_cache.get('A'), UNDEFINED)
        self.assertEquals(ws.resource_cache.get('Y'), UNDEFINED)
        self.assertEquals(ws.resource_cache.get('Z'), UNDEFINED)

        ws.execute_workflow()
        self.assertEquals(ws.resource_cache.get('X', '--'), '--')
        self.assertEquals(ws.resource_cache.get('A'), 5)
        self.assertEquals(ws.resource_cache.get('Y'), 5)
        self.assertEquals(ws.resource_cache.get('Z'), 5)
    ...
    def test_set_step_and_run_op(self):
        ...
        self.assertEquals(len(ws.workflow.steps), 1)
        self.assertIn('X', ws.resource_cache)
        ...
        self.assertEquals(len(op_result), 4)
    ...
    def test_set_resource_is_reentrant(self):
        ...
        self.assertEquals(actual_res_names, expected_res_names)
    ...
    def test_example(self):
        ...
        self.maxDiff = None
        self.assertEquals(ws.workflow.to_json_dict(), expected_json_dict)
        ...
        self.assertEquals(ws2.workflow.op_meta_info.qualified_name, ws.workflow.op_meta_info.qualified_name)
        self.assertEquals(len(ws2.workflow.steps), len(ws.workflow.steps))
